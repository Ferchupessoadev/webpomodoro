...
export default function toggleButtonsTodosTask(element) {
    ...
    switch (element.getAttribute('id')) {
        case 'cbx-51':
            if (todoListToday.children[2].children.length > 0) {
                element.checked
                    ? (todoListToday.style.display = 'none')
                    : (todoListToday.style.display = 'block');
            }
            break;
        case 'cbx-52':
            if (todoListFuture.children[2].children.length > 0) {
                element.checked
                    ? (todoListFuture.style.display = 'none')
                    : (todoListFuture.style.display = 'block');
            }
            break;
        case 'cbx-53':
            if (todoListPast.children[2].children.length > 0) {
                element.checked
                    ? (todoListPast.style.display = 'none')
                    : (todoListPast.style.display = 'block');
            }
            break;
        case 'cbx-54':
            if (todoListCompleted.children[2].children.length > 1) {
                element.checked
